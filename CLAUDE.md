...
Tournament data lives in `tournament/`. YAML files are in `tournament/items/`, named `tournament-YYYY-MM-DD.yml` (date = tournament start). The `tournament/legacy.csv` is a historical flat database (1634 players, reward columns T1–T5, RT1–RT4, TF0–TF22) — it is not processed by the pipeline; it's reference data. The `tournament/classic/classic-rewards.yml` is the YAML migration of `legacy.csv`, generated by `scripts/migrate-classic-rewards.ts`.
